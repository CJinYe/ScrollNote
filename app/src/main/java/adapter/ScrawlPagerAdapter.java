...
public class ScrawlPagerAdapter extends FragmentStatePagerAdapter {
    ...
    public Fragment getItem(int position) {
        ...
        if (mDesFiles != null && position < mDesFiles.size()) {
            ...
            if (mZipPath != null) {
                NoteBean noteBean = mNoteDbDao.queryNoteZipPaht(mZipPath + ".zip", mDesFiles.get(position).getName());
                if (noteBean != null && !TextUtils.isEmpty(noteBean.text)) {
                    mNoteFragment.setText(noteBean.text);
                }
            } else {
                NoteBean noteBean = mNoteDbDao.queryNotePaht(mDesFiles.get(position).getPath());
                if (noteBean != null && !TextUtils.isEmpty(noteBean.text)) {
                    mNoteFragment.setText(noteBean.text);
                }
            }
