...
import utils.BottomNavigationUtils;
...
//    private final int NAVIGATION_HEIGHT = (int) getResources().getDimension(com.luseen.luseenbottomnavigation.R.dimen.bottom_navigation_height);
    private  int NAVIGATION_HEIGHT = 120;
...
//        NAVIGATION_HEIGHT = heightMeasureSpec;
...
//        setMeasuredDimension(measuredHeight, measuredWidth);
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        switch (specMode) {
            case MeasureSpec.AT_MOST:  // 子容器可以是声明大小内的任意大小
                Log.e(Tag, "子容器可以是声明大小内的任意大小"+heightMeasureSpec);
                Log.e(Tag, "大小为:"+specSize);
                result=specSize;
                break;
            case MeasureSpec.EXACTLY: //父容器已经为子容器设置了尺寸,子容器应当服从这些边界,不论子容器想要多大的空间.  比如EditTextView中的DrawLeft
                Log.e(Tag, "父容器已经为子容器设置了尺寸,子容器应当服从这些边界,不论子容器想要多大的空间"+heightMeasureSpec);
                Log.e(Tag, "大小为:"+specSize);
                result=specSize;
                break;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        for (int i = 0; i < bottomNavigationItems.size(); i++) {
            ...
            final View view = inflater.inflate(R.layout.item_bottom_menu, this, false);
...
//        }
        ...
        int viewActivePaddingTop = 4;
        int viewInactivePaddingTop = 7;
        int viewInactivePaddingTopWithoutText = 10;
        int centerX;
        int centerY;
